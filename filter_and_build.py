...
BOT = "NimsiluBot"   # only NimsiluBot
MIN_ELO = 2250
MAX_BOOK_PLIES = 50
...
def fetch_games(username, filename):
    """Fetch all Crazyhouse games for a bot (any opponent)"""
    url = f"https://lichess.org/api/games/user/{username}"
    params = {
        "max": 200000,
        "perfType": "crazyhouse",   # <-- changed
        "rated": "true",
        "evals": "false",
        "clocks": "false",
        "opening": "true",
...
def filter_games(input_file, output_file):
    """Filter PGN: Crazyhouse, NimsiluBot rated ≥MIN_ELO"""
    ...
    with open(input_file, "r", encoding="utf-8") as f, open(output_file, "w", encoding="utf-8") as out:
        for line in f:
            ...
            if white_name == BOT and white_rating >= MIN_ELO:
                if "pgn" in game:
                    out.write(game["pgn"] + "\n\n")
            elif black_name == BOT and black_rating >= MIN_ELO:
                if "pgn" in game:
                    out.write(game["pgn"] + "\n\n")
...
def build_polyglot_book(pgn_path, book_path):
    ...
    with open(pgn_path, "r", encoding="utf-8") as pgn_file:
        for i, game in enumerate(iter(lambda: chess.pgn.read_game(pgn_file), None), start=1):
            if game is None:
                break
            if game.headers.get("Variant", "").lower() != "crazyhouse":  # <-- changed
                continue
...
# ---------- MAIN ----------
def main():
    ndjson_file = f"{BOT}_crazyhouse.ndjson"
    ...
    # Build book
    build_polyglot_book(pgn_file, "crazyhouse_book.bin")
